...
//*****************************************************************************
//
// This function implements the "wte" command.  It simply writes to a text file
// working directory.
//
//*****************************************************************************
int
Cmd_wte(int argc, char *argv[])
{
    ...
    iFResult = f_open(&fil, "temperature.txt", FA_CREATE_NEW|FA_WRITE);
    SysCtlDelay(SysCtlClockGet()/3);
    if(iFResult != FR_OK) {UARTprintf("fresult: %s\n", StringFromFResult(iFResult)); }
    else{UARTprintf("\n Opened SD card for writing\n");}
...
tCmdLineEntry g_psCmdTable[] =
{
    ...
    { "cr",     Cmd_wte,    "Create a text file Hello_World.txt and write to it" },
    { 0, 0, 0 }
};
